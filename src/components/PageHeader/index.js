  ...
  render() {
    ...
    if (routes && params) {
      ...
      pathSnippets.map((_, index) => {
        ...
        Object.keys(breadcrumbNameMap).forEach(x => {
          if (pathToRegexp(x).exec(url) || pathToRegexp(x).exec(`/${url}`)) {
            pathArr.push(`/${url}`);
          }
        });
      });
      /*************************/
      const extraBreadcrumbItems = pathArr.map((url, index) => {
        return (
          <Breadcrumb.Item key={url}>
            {createElement(
              index === pathArr.length - 1 ? 'span' : linkElement,
